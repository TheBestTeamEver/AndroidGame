...
public class QuestionProcessor {
    ...
    public static QuestionSet processQuest(final String text) throws IOException {
        ...
        if (msg != null) {
            String firstImageUrl = "http://91.218.230.80" + msg.getData().get(level).get("true");
            String secondImageUrl = "http://91.218.230.80" + msg.getData().get(level).get("fake");
            String personName = msg.getData().get(level).get("name");
            level++;
            ...
            return new QuestionSet(firstImg, secondImg, personName, true);
        }
        return null;
    }
